#ifndef DUNE_VOF_INTERFACENORMALDIVERGENCECURVATURE_HH
#define DUNE_VOF_INTERFACENORMALDIVERGENCECURVATURE_HH
...
namespace Dune
{
  namespace VoF
  {
    ...
    // Calculate the curvature of the inferface in each mixed cell by the divergence of the interface normals.
    ...
    struct InterfaceNormalDivergenceCurvature
    {
      using GridView = GV;
      using Stencils = ST;
    ...
    private:
      ...
      void applySmoothing1st ( const Entity &entity, const DiscreteFunction &uh, const CurvatureSet &tmpCurvature, CurvatureSet &curvature  )
      {
        ...
        {
          kappa += curvature[ neighbor ] * smoothingWeight( uh[ neighbor ] );
      ...
      void applySmoothing2nd ( const Entity &entity, const DiscreteFunction &uh, const ReconstructionSet &reconstructions, const CurvatureSet &tmpCurvature, CurvatureSet &curvature  )
      {
        ...
        for ( const auto &neighbor : stencil( entity ) )
        {
          ...
          kappa += curvature[ neighbor ] * smoothingWeight( uh[ neighbor ] ) * additionalSmoothingWeight( normal, delta );
...
#endif // #ifndef DUNE_VOF_INTERFACENORMALDIVERGENCECURVATURE_HH
